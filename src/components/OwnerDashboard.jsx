...
export const OwnerDashboard = () => {
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      {myShopData && 
       <div className="w-full flex flex-col items-center gap-6 px-4 sm:px-6">
         ...
         <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-orange-100 hover:shadow-2xl transition-all duration-300 w-full max-w-3xl relative">
          <span onClick={()=> navigate("/create-edit-shop")} className="absolute top-4 right-4 bg-orange-500 text-white p-2 rounded-full shadow-md hover:bg-orange-600 transition-colors cursor-pointer">
            <FaPen />
          </span>
      ...
      <div className="flex justify-center items-center p-4 sm:p-6 shadow-lg hover:shadow-2xl transition-all duration-300">
        <div className="w-full max-w-md bg-white shadow-lg rounded-2xl p-6 border-gray-100 hover:shadow-xl transition-shadow duration-300">
          <div className="flex flex-col items-center text-center">
            <FaUtensils className="text-[#ff4d2d] w-16 h-16 sm:w-20 sm:h-20 mb-4"  />
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">Add Your Food Items</h2>
            <p className="text-gray-600 mb-4 text-sm sm:text-base">Share Your delicious creations with our custumers by adding them to the menu.</p>
            <button onClick={()=> navigate("/add-item")} className="bg-[#ff4d2d] text-white px-5 sm:px-6 py-2 rounded-full font-medium shadow-md hover:bg-orange-600 transition-colors duration-200 cursor-pointer">Add Food</button>
          </div>
        </div>
      </div>}
